...
## raw data with NA containing 0, 1 and ., COL = #CHROM/POS/206 individuals, ROW = SNP Position 
...
data_test<-as.data.frame(c(row_WK[,1:2], row_nb, col_nb, row_WK[[col_nb]]), col.names = c("#CHROM","POS","row_nb","col_nb", "IP"))

  if (is.na(data_test$IP) == FALSE) { # If value ==! NA, save the value in a new dataset data_save
...
data_imputed <- as.data.frame(t(data_imputed)) ##COL=Individuals et ROW==SNPs
